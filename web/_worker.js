...
async function api(request, env) {
  ...
  /* ---------- STUDENT SUBJECT VERIFICATION ---------- */
  if ((path === "/api/v1/user/verify-subjects" || path === "/api/v1/auth/verify-lms-challenge") && method === "POST") {
    const body = await parse(request);
    const studentId = String(body.student_id || (me ? (me.student_id || me.uid) : "") || "").toUpperCase().trim();
    const rawCourses = Array.isArray(body.courses) ? body.courses : (Array.isArray(body.verified_courses) ? body.verified_courses : (Array.isArray(body.enrolled_subjects) ? body.enrolled_subjects : []));
    const verifiedCourses = rawCourses.map(c => courseCode(String(c))).filter(Boolean);

    if (!studentId || verifiedCourses.length === 0) {
      return J(request, { ok: false, error: "Student ID and valid courses required" }, 400);
    }

    const payload = {
      user_id: me ? me.uid : studentId,
      student_id: studentId,
      student_name: body.student_name || "VU Student",
      vu_email: body.vu_email || `${studentId.toLowerCase()}@vu.edu.pk`,
      program: body.program || "BS",
      semester: body.semester || "Current",
      verified_subjects: verifiedCourses,
      is_verified: true,
      verified_via: body.method || (body.extension_verified ? "lms_extension" : "vulms_session"),
      verified_at: now()
    };

    try {
      await sb(env, 'user_settings', `?user_id=eq.verify_${studentId}`, {
        method: 'POST',
        body: {
          user_id: `verify_${studentId}`,
    ...
    return J(request, { ok: true, verified: true, student_id: studentId, courses: verifiedCourses, profile: payload });
  }

  if (path.startsWith("/api/v1/user/verified-status/") && method === "GET") {
    const sid = decodeURIComponent(path.replace("/api/v1/user/verified-status/", "")).toUpperCase().trim();
    let isVer = false;
    let vCourses = [];
    try {
      const rows = await sb(env, 'user_settings', `?user_id=eq.verify_${sid}&select=settings_json`);
      if (Array.isArray(rows) && rows[0] && rows[0].settings_json) {
        const data = JSON.parse(rows[0].settings_json);
        isVer = data.is_verified === true;
        vCourses = data.verified_subjects || [];
      }
    } catch (_) {}
    return J(request, { ok: true, verified: isVer, courses: vCourses });
  }
