...
async function api(request, env) {
  ...
  if ((path === "/api/v1/ai" || path === "/api/v1/ai/ask") && method === "POST") {
    ...
    if (mode === "mcq_explain" || (question && options.length > 0)) {
      ...
      // 1. Check Gemini API key pool in env (supports GEMINI_API_KEY, GEMINI_API_KEY_1 to _15)
      const geminiKeys = Object.keys(env)
        .filter(k => /^GEMINI_API_KEY(?:_\d+)?$/i.test(k) || k === 'GOOGLE_AI_KEY')
        .map(k => env[k])
      ...
      if (geminiKeys.length > 0) {
        ...
        for (const geminiKey of geminiKeys) {
          for (const modelName of models) {
            try {
              const gRes = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${modelName}:generateContent?key=${geminiKey}`, {
                method: "POST",
              ...
              console.warn("Gemini AI API notice:", modelName, gErr);
            }
          }
        }
      }
